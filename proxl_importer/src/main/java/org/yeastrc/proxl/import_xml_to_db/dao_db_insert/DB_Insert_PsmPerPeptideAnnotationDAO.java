...
public class DB_Insert_PsmPerPeptideAnnotationDAO {
	...
	/**
	 * This will INSERT the given PsmPerPeptideAnnotationDTO into the database.
	 * @param item
	 * @throws Exception
	 */
	public void saveToDatabase( PsmPerPeptideAnnotationDTO item ) throws Exception {
...
//			}
	 ...
	 * @throws Exception
	 */
	public void saveToDatabase( PsmPerPeptideAnnotationDTO item, Connection conn ) throws Exception {
		...
		try {
			...
			if ( item.getAnnotationValueLocation() == AnnotationValueLocation.LARGE_VALUE_TABLE ) {
				DB_Insert_PsmPerPeptideAnnotationLargeValueDAO.getInstance().saveToDatabase( item.getId(), item.getValueString(), conn );
			}
			...
			log.error( "ERROR: If length of field value_string is exceeded, update Java class AnnotationValueStringLocalFieldLengthConstants. sql: " + INSERT_SQL
					+ ".  PsmPerPeptideAnnotationDTO item: " + item, e );
	...
	private final static String INSERT_SQL = 
			"INSERT INTO psm_per_peptide_annotation "
			
			+ "(psm_id, srch_rep_pept__peptide_id, filterable_descriptive_type, annotation_type_id, value_location, value_double, value_string ) "
	 ...
	 * @throws Exception
	 */
	private void saveToDatabaseInternal( PsmPerPeptideAnnotationDTO item, Connection conn ) throws Exception {

...
function Peptide(seq, staticModifications, varModifications, ntermModification, ctermModification, maxNeutralLossCount, loopLinkData, crossLinkData) {
    ...
    this.getNeutralMassMono = function _massNeutralMono() {
        ...
        mass = _addCrossLinkLoopLinkModMasses(mass, sequence.length, "n", "mono");
        // add N-terminal H
    ...
    this.getNeutralMassAvg = function _massNeutralAvg() {
            ...
            mass = _addTerminalModMass(mass, "c");
            mass = _addResidueModMasses(mass, sequence.length, "n");
            mass = _addCrossLinkLoopLinkModMasses(mass, sequence.length, "n", "avg");
            // add N-terminal H
...
//    };
    ...
    function _addCrossLinkLoopLinkModMasses(seqMass, index, term, massType) {
        ...
        if ( loopLinkData || crossLinkData ) {

        } else {
        	
        	//  Exit if not have object for loopLinkData or crossLinkData
        	
        	return seqMass; // EARLY EXIT
        }
    	
        var mass = seqMass;
        	
        var slice = new _Slice(index, term);
        for( var sliceIndex = slice.from; sliceIndex < slice.to; sliceIndex += 1) {

        	var sliceIndexStartAtOne = sliceIndex + 1;
        	
        	if ( loopLinkData ) {
            	
            	//  Add in Loop Link Linker Mass at loopLinkPos1

            	if ( sliceIndexStartAtOne == loopLinkData.loopLinkPos1 ) {
            		
            		mass += loopLinkData.linkerMass;
            	}
            }
    ...
    function _getSeqMass(index, term, massType) {
        ...
        mass = _addCrossLinkLoopLinkModMasses(mass, index, term, massType);
        return mass;
    }
...
//    Loop link data for Cross Linking projects

function LoopLinkData( loopLinkDataInputFormat ) { //  loopLinkData
	
	this.loopLinkPos1 = loopLinkDataInputFormat.loopLinkPos1;
	this.loopLinkPos2 = loopLinkDataInputFormat.loopLinkPos2;
	this.linkerMass = loopLinkDataInputFormat.linkerMass;
}
...
CrossLinkData.prototype.getLinkerPosition = function() {
	
	if ( this.currentPeptideNumber == 1 ) {
		
		return this.crossLinkPos1;
	} else {
		
		return this.crossLinkPos2;
	}
};
...
CrossLinkData.prototype.getCrossLinkedPeptideAndLinkerMass = function( massType ) {
	...
	if(massType == 'mono')
		otherPeptideMass += this.otherPeptide.getNeutralMassMono();
     else
    	 otherPeptideMass += this.otherPeptide.getNeutralMassAvg();

...
//			+ "			AND c.search_id IN (#SEARCHES#) AND b.nrseq_id = ? AND b.protein_position_1 = ? AND b.protein_position_2 = ?";
	...
	public List<ReportedPeptideDTO> getPeptides( MergedSearchProteinLooplink looplink ) throws Exception {
		...
		Collection<SearchDTO> searchesParam = looplink.getSearches();

		SearcherCutoffValuesRootLevel searcherCutoffValuesRootLevel = looplink.getSearcherCutoffValuesRootLevel();
...
//			sql = sql.replaceAll( "#SEARCHES#", StringUtils.join( searchIds, "," ) );
			...
			setPstmtQueryParameters( looplink, cutoffsPerSearchHolderList, pstmt );
...
//					AND c.search_id IN (#SEARCHES#) AND b.nrseq_id = ? AND b.protein_position_1 = ? AND b.protein_position_2 = ?";
	...
	/**
	 * Get the number of distinct peptides (that is, distinct pair of looplinked peptides) found that identified the given looplinked proteins/positions
	 * @param looplink
	 * @return
	 * @throws Exception
	 */
...
//			sql = sql.replaceAll( "#SEARCHES#", StringUtils.join( searchIds, "," ) );
	 ...
	 * @throws SQLException
	 */
	private void setPstmtQueryParameters(MergedSearchProteinLooplink looplink,
			List<CutoffsPerSearchHolder> cutoffsPerSearchHolderList,
		...
		for ( CutoffsPerSearchHolder cutoffsPerSearchHolder : cutoffsPerSearchHolderList ) {
			...
			paramCounter++;
			pstmt.setInt( paramCounter, looplink.getProtein().getNrProtein().getNrseqId() );
			paramCounter++;
			pstmt.setInt( paramCounter, looplink.getProteinPosition1() );
			paramCounter++;
			pstmt.setInt( paramCounter, looplink.getProteinPosition2() );

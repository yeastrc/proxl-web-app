...
public class SearchProteinCrosslinkSearcher {
	...
	private static final Logger log = Logger.getLogger( SearchProteinCrosslinkSearcher.class );
...
//	private final boolean USE_PEPTIDE_PSM_DEFAULTS_TO_SKIP_JOIN_ANNOTATION_DATA_VALUES_TABLES = true;  //  UNTESTED for a value of "true"
	...
	private final String SQL_SEARCH_ON_SEARCH_ID_FIRST_PART = 
		...
		"SELECT search_crosslink_generic_lookup.nrseq_id_1, "
	 ...
	 * @throws Exception
	 */
	public List<SearchProteinCrosslinkWrapper> searchOnSearchIdandCutoffs( SearchDTO search, SearcherCutoffValuesSearchLevel searcherCutoffValuesSearchLevel ) throws Exception {
		...
		try {
			...
			//   For:  search_crosslink_generic_lookup.search_id = ? 
			...
			while( rs.next() ) {
				...
				SearchProteinCrosslinkWrapper wrappedLink = new SearchProteinCrosslinkWrapper();
	...
	////////////     Retrieval of single SearchProteinCrosslink based on provided criteria

	private final String SQL_SEARCH_ON_SEARCH_ID_CROSS_LINK_FIRST_PART = 

		"SELECT search_crosslink_generic_lookup.num_psm_at_default_cutoff, "
			+ " search_crosslink_generic_lookup.num_linked_peptides_at_default_cutoff, "
			+ " search_crosslink_generic_lookup.num_unique_peptides_linked_at_default_cutoff ";
	...
	private static final String SQL_SEARCH_ON_SEARCH_ID_CROSS_LINK_WHERE_START =  
			" WHERE search_crosslink_generic_lookup.search_id = ?   "
			+ " AND search_crosslink_generic_lookup.nrseq_id_1 = ? "
			+ " AND search_crosslink_generic_lookup.nrseq_id_2 = ? "
	 ...
	 * @throws Exception
	 */
	public SearchProteinCrosslinkWrapper search( 
			SearchDTO search, 
		...
		SearchProteinCrosslinkWrapper wrappedLink = null;
		...
		try {
			...
			if( rs.next() ) {
				...
				wrappedLink = new SearchProteinCrosslinkWrapper();

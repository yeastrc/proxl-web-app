...
public class SearchPeptideDimerSearcher {
	
	private static final Logger log = Logger.getLogger(SearchPeptideDimerSearcher.class);

	private SearchPeptideDimerSearcher() { }
	private static final SearchPeptideDimerSearcher _INSTANCE = new SearchPeptideDimerSearcher();
	public static SearchPeptideDimerSearcher getInstance() { return _INSTANCE; }
	

	
	/**
	 * Get the SearchPeptideDimer for the given peptide in the given search with the given search parameters
	 * @param search
	 * @param psmCutoff
	 * @param peptideCutoff
	 ...
	 * @throws Exception
	 */
	public SearchPeptideDimer searchOnSearchIdPeptideIdPsmCutoffPeptideCutoff( SearchDTO search, double psmCutoff, double peptideCutoff, int reportedPeptideId ) throws Exception {
		
		
		SearchPeptideDimer link = null;
		...
		try {
			...
			pstmt.setString( 5, XLinkUtils.getTypeString( XLinkUtils.TYPE_DIMER ) );
			
			rs = pstmt.executeQuery();

			if( rs.next() ) {
				link = new SearchPeptideDimer();

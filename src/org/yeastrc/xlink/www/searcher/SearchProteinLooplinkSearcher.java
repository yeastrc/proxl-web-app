...
public class SearchProteinLooplinkSearcher {
	...
	private static final Logger log = Logger.getLogger( SearchProteinLooplinkSearcher.class );
...
//	private final boolean USE_PEPTIDE_PSM_DEFAULTS_TO_SKIP_JOIN_ANNOTATION_DATA_VALUES_TABLES = true;  //  UNTESTED for a value of "true"
	...
	private final String SQL_SEARCH_ON_SEARCH_ID_FIRST_PART = 
		...
		"SELECT search_looplink_generic_lookup.nrseq_id, "
			...
			+ " search_looplink_generic_lookup.num_psm_at_default_cutoff, "
			+ " search_looplink_generic_lookup.num_linked_peptides_at_default_cutoff, "
			+ " search_looplink_generic_lookup.num_unique_peptides_linked_at_default_cutoff ";

	private final String SQL_SEARCH_ON_SEARCH_ID_FROM_START = 	
			
			" FROM search_looplink_generic_lookup";
	 ...
	 * @throws Exception
	 */
	public List<SearchProteinLooplinkWrapper> searchOnSearchIdandCutoffs( SearchDTO search, SearcherCutoffValuesSearchLevel searcherCutoffValuesSearchLevel ) throws Exception {
		...
		try {
			...
			//   For:  search_looplink_generic_lookup.search_id = ? 
			...
			while( rs.next() ) {
				...
				SearchProteinLooplinkWrapper wrappedLink = new SearchProteinLooplinkWrapper();
	...
	private final String SQL_SEARCH_ON_SEARCH_ID_LOOPLINK_FIRST_PART = 
			...
			"SELECT"
//					+ " search_looplink_generic_lookup.nrseq_id, "
//					+ " search_looplink_generic_lookup.protein_position_1, "
//					+ " search_looplink_generic_lookup.protein_position_2, "
	...
	private static final String SQL_SEARCH_ON_SEARCH_ID_LOOPLINK_WHERE_START =  
			" WHERE search_looplink_generic_lookup.search_id = ? "
			+ " AND search_looplink_generic_lookup.nrseq_id = ?"
	 ...
	 * @throws Exception
	 */
	public SearchProteinLooplinkWrapper search( 
			SearchDTO search, 
		...
		SearchProteinLooplinkWrapper wrappedLink = null;
		...
		try {
			...
			//   For:  search_looplink_generic_lookup.search_id = ? AND nrseq_id = ? AND protein_position_1 = ? AND protein_position_2 = ?  ";
			...
			if( rs.next() ) {
				...
				wrappedLink = new SearchProteinLooplinkWrapper();
	...
	public String getSQL(
		...
		
		{
			if ( ( ( ! onlyDefaultPsmCutoffs ) || ( ! onlyDefaultPeptideCutoffs ) )
				...
				for ( int counter = 1; counter <= psmCutoffValuesList.size(); counter++ ) {
					...
					sqlSB.append( " search_looplink_best_psm_value_generic_lookup AS best_psm_value_tbl_" );
					sqlSB.append( Integer.toString( counter ) );

					sqlSB.append( " ON "  );

					
					sqlSB.append( " search_looplink_generic_lookup.search_id = "  );
					...
					sqlSB.append( " search_looplink_generic_lookup.nrseq_id = "  );
					...
					sqlSB.append( " search_looplink_generic_lookup.protein_position_1 = "  );

					sqlSB.append( "best_psm_value_tbl_" );
					sqlSB.append( Integer.toString( counter ) );
					sqlSB.append( ".protein_position_1" );

					sqlSB.append( " AND " );

					sqlSB.append( " search_looplink_generic_lookup.protein_position_2 = "  );

					sqlSB.append( "best_psm_value_tbl_" );
					sqlSB.append( Integer.toString( counter ) );
					sqlSB.append( ".protein_position_2" );
		...
		
		{
			if ( ( ( ! onlyDefaultPsmCutoffs ) || ( ! onlyDefaultPeptideCutoffs ) )
				...
				for ( int counter = 1; counter <= peptideCutoffValuesList.size(); counter++ ) {
					...
					sqlSB.append( " search_looplink_best_peptide_value_generic_lookup AS best_rep_pept_value_tbl_" );
					sqlSB.append( Integer.toString( counter ) );
...

		{
			if ( ( ( ! onlyDefaultPsmCutoffs ) || ( ! onlyDefaultPeptideCutoffs ) )
				...
				sqlSB.append( " search_looplink_generic_lookup.num_psm_at_default_cutoff > 0 " );
...

		{
			if ( ( ( ! onlyDefaultPsmCutoffs ) || ( ! onlyDefaultPeptideCutoffs ) )
				...
				sqlSB.append( " search_looplink_generic_lookup.num_linked_peptides_at_default_cutoff > 0 " );
			}
		}		

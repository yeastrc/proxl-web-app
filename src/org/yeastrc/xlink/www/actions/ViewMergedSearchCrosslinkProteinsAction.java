...
public class ViewMergedSearchCrosslinkProteinsAction extends Action {

	private static final Logger log = Logger.getLogger(ViewMergedSearchCrosslinkProteinsAction.class);
...
//			HttpSession session = request.getSession();
			...
			//Collection<MergedSearchProtein> prProteins = MergedSearchProteinSearcher.getInstance().getProteinsWithLinkType(searches, XLinkUtils.TYPE_CROSSLINK, psmQValueCutoff, peptideQValueCutoff);
...
//					// did they request to removal of links with only one PSM?
			...
			List<MergedSearchProteinCrosslinkWrapper> wrappedCrossLinks = new ArrayList<MergedSearchProteinCrosslinkWrapper>( crosslinks.size() );
			...
			request.setAttribute( "crosslinks", wrappedCrossLinks );
			...
			VennDiagramDataToJSON vennDiagramDataToJSON =
					GenerateVennDiagramDataToJSON.createVennDiagramDataToJSON( crosslinks, searches );
			...
			for( IMergedSearchLink link : crosslinks ) {

...
//		console.log("processSortUpdateAnnotationDisplayItems");
		...
		var searchId = $overlayContainerBlock.attr("data-search_id");
		if ( searchId === undefined || searchId === null || searchId === "" ) {
			throw Error( "attr 'data-search_id' missing or empty string" );
		}
		...
		var selectedAnnTypesForSearchId = selectedAnnTypesForSearches[ searchId ];
		if ( selectedAnnTypesForSearchId === undefined ) {
			throw Error( "selectedAnnTypesForSearchId not found for searchId: " + searchId );
		}
		
		//  reset to empty array and will populate next while processing sorted DOM elements
		
		selectedAnnTypesForSearchId[ psmPeptideTypeString ] = [];
		
		var selectedDataEntries = selectedAnnTypesForSearchId[ psmPeptideTypeString ];
	...
	this.putAnnTypeIdDisplayOnThePage = function( params ) {
		...
		try {
			...
			var processedSearchIds = {};
			
			$per_search_container_jq_All.each( function() {

				var $per_search_container_jq_Each = $( this );
				var search_id = $per_search_container_jq_Each.attr("data-search_id");
				if ( search_id === undefined || search_id === null || search_id === "" ) {
					throw Error( "attr 'data-search_id' missing or empty string" );
				}

				var annDisplayItemsForSearch = annDisplayItemsSearches[ search_id ];
				if ( annDisplayItemsForSearch === undefined || annDisplayItemsForSearch === null ) {
					throw Error( "No data found in input params for search_id: " + search_id );
				}
				...
				$overlayContainerBlock_All.each( function() {
					...
					if ( psmAnnDisplayItemsForSearch === undefined || psmAnnDisplayItemsForSearch === null ) {
						throw Error( "No data found in input params for search_id: " + search_id + " for property 'psm'" );
					}
...
//					objectThis._updateAnnDataDisplayMainDisplay( { $overlayContainerBlock : $overlayContainerBlock_Each } );
					...
					processedSearchIds[ search_id ] = true;
			...
			var annDisplayItemsSearchIdArray = Object.keys( annDisplayItemsSearches );
			for ( var annDisplayItemsSearchIdIndex = 0; annDisplayItemsSearchIdIndex < annDisplayItemsSearchIdArray.length; annDisplayItemsSearchIdIndex++ ) {
				var annDisplayItemsSearchId = annDisplayItemsSearchIdArray[ annDisplayItemsSearchIdIndex ];
				if ( ! processedSearchIds[ annDisplayItemsSearchId ] ) {
					throw Error( "Search Id in input params '" + annDisplayItemsSearchId + "' not found on the page." );
				}
			}
	...
	this.getAnnotationTypeDisplayFromThePage = function( params ) {
		...
		try {
			...
			var resultObj = { 
					...
					annTypeIdDisplayBySearchId : _annTypeIdDisplaySavedValues
			};
...
//			var $clickedThis = $( clickedThis );
	...
	this.searchSelectorChanged = function( params ) {
		try {
			...
			var elementId = "annotation_displays_modal_dialog_overlay_div_search_id_" + searchId;
...
//			this._updateAnnDataDisplayMainDisplay( { $overlayContainerBlock : $overlayContainerBlock } );
	...
	this.reloadDisplayedDataOnImageAndStructurePages = function( params ) {
		...
		$main_data_row_jq_All.each( function( index, element ) {
			...
			var children_mgmt_objectString = $main_data_row_jq_Each.attr("children_mgmt_object");

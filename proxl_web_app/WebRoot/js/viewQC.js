...
var ViewQCPageCode = function() {
	...
	var _scan_Statistics_sectionDisplayed = false;
...
//			this.showScanLevelstatistics();
	...
	this._scan_level_collapse_link_Clicked = function( params ) {
		this.hideScanLevelStatistics();
	};

	/**
	 *  
	 */
	this.showScanLevelstatistics = function( params ) {

		_scan_Statistics_sectionDisplayed = true;
		...
		this.load_Scan_Level_StatisticsIfNeeded();
	};
	
	/**
	 *  
	 */
	this.hideScanLevelStatistics = function( params ) {
		...
		_scan_Statistics_sectionDisplayed = false;
	};
	...
	this.clearAllDisplayedDataAndCharts = function() {
		...
		this.clear_Scan_Level_Statistics();
	};
	...
	this.loadDataForDisplayedDataAndCharts = function() {
		...
		if ( _scan_Statistics_sectionDisplayed ) {
			this.load_Scan_Level_StatisticsIfNeeded();
		}
	};
...
//		});
	...
	/**
	 * Clear the data for Scan 
	 */
	this.clear_Scan_Level_Statistics = function() {
	
		this.clearScanFileSelector();
	...
	/**
	 * Load the data for Scan 
	 */
	this.load_Scan_Level_StatisticsIfNeeded = function() {
	
		this.loadScanFileSelectorIfNeeded();
	};
	 ...
	 */
	this.loadScanFileSelector = function() {
		var objectThis = this;
		var _URL = contextPathJSVar + "/services/utils/getScanFilesForProjectSearchId";
		var requestData = {
				projectSearchId : _project_search_ids
		};
		//  Skip Save request since never abort
//		var request =
		$.ajax({
			type : "GET",
			url : _URL,
			...
			success : function(data) {
				try {
					objectThis.loadScanFileSelectorResponse(requestData, data);
				} catch( e ) {
					reportWebErrorToServer.reportErrorObjectToServer( { errorException : e } );
					throw e;
				}
	...
	/**
	 * Process AJAX response for loadScanFileSelector
	 */
	this.loadScanFileSelectorResponse = function(requestData, responseData) {
//		var objectThis = this;

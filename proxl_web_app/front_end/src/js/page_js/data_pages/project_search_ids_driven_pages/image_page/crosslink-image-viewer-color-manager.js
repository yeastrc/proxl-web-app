...
ColorManager.prototype.getColorForIndex = function( index ) {
	var uid = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_indexManager().getUIDForIndex( index );
...
ColorManager.PrettyTheme.prototype.getColorForLink = function( link ) {	
	...
	if( !imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_imageProteinBarDataManager().isAnyProteinBarsHighlighted() ) {	
		return this.getColorForHighlightedLink( link );		
	}
	
	//  Check exclusions
	if( link.type === "crosslink" ) {
		if ( imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_linkExclusionDataManager().isLinkExcludedForProteinUIDProteinPosition( {
	...
	var barEntry1 = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_imageProteinBarDataManager().getItemByUID( link.uid1 );
	var barEntry2 = undefined;			//undefined if monolink or looplink
	if( link.type === "crosslink" ) {
		barEntry2 = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_imageProteinBarDataManager().getItemByUID( link.uid2 );		
	}
...
ColorManager.PrettyTheme.prototype.getColorForUIDAnnotation = function( uid ) {
	...
	var barData = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_imageProteinBarDataManager().getItemByUID( uid );
	
	if( imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_colorLinesBy() !== imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_SELECT_ELEMENT_COLOR_BY_REGION() ) {
	...
	if( imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_imageProteinBarDataManager().isAnyProteinBarsHighlighted() ) {
		...
		if( barData.isAllOfProteinBarHighlighted() ) {
			...
			if( imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_colorLinesBy() === imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_SELECT_ELEMENT_COLOR_BY_REGION() ) {
				...
				region.uid = uid;
				region.start = 1;
				region.end = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_proteinLengths().getProteinLength( imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_indexManager().getProteinIdForUID( uid ) );
...
ColorManager.PrettyTheme.prototype.getColorForHighlightedLink = function( link ) {
	...
	// color by search
	if( imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_colorLinesBy() === imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_SELECT_ELEMENT_COLOR_BY_SEARCH() ) {

		color =  this.getColorForLinkUsingSearch( link );
		
	} else if( imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_colorLinesBy() === imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_SELECT_ELEMENT_COLOR_BY_REGION() ) {
...
ColorManager.PrettyTheme.prototype.getColorForLinkUsingSearch = function ( link ) {
	...
	if( link.type === "crosslink" ) { searches = imagePagePrimaryRootCodeObject_LocalCopy.call__findSearchesForCrosslink( link.protein1, link.protein2, link.position1, link.position2 ); }
	else if( link.type === "looplink" ) { searches = imagePagePrimaryRootCodeObject_LocalCopy.call__findSearchesForLooplink( link.protein1, link.position1, link.position2 ); }
	else if( link.type === "monolink" ) { searches = imagePagePrimaryRootCodeObject_LocalCopy.call__findSearchesForMonolink( link.protein1, link.position1 ); }
...
ColorManager.PrettyTheme.prototype.getColorForSearches = function( searches ) {
	...
	for ( var i = 0; i < imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_searches().length; i++ ) {
		for ( var k = 0; k < searches.length; k++ ) {
			if ( imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_searches()[i]['id'] === searches[ k ] ) {
				colorIndex += ( i + 1 );
				break;
			}
		}
...
ColorManager.PrettyTheme.prototype.getColorForHighlightedUID = function( uid ) {
	...
	var index = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_indexManager().getIndexForUID( uid );
		
	if( index < 0 ) {
		throw Error( "Could not find uid: " + uid + " in index manager." );
	}
	
	var number = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_indexManager().getProteinList().length;
...
ColorManager.getAllRegions = function() {
	...
	var entries = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_indexManager().getProteinArray();
	var regions = [ ];
	...
	for( var i = 0; i < entries.length; i++ ) {
		...
		if( !imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_imageProteinBarDataManager().isAnyProteinBarsHighlighted() ) {	
			...
			region.uid = uid;
			region.start = 1;
			region.end = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_proteinLengths().getProteinLength( pid );
	...
	if ( $( "input#shade-by-counts" ).is( ':checked' ) ) {
		...
		try {
			if( link.type == 'crosslink' ) {
				...
				numPsms = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_linkPSMCounts()[ 'crosslink' ][ link.protein1 ][ link.protein2 ][ link.position1 ][ link.position2 ];				

			} else if( link.type == 'looplink' ) {

				try {
					numPsms = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_linkPSMCounts()[ 'looplink' ][ link.protein1 ][ link.protein1 ][ link.position1 ][ link.position2 ];
				} catch( err ) { }
				
				if( !numPsms ) {
					try {
						numPsms = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_linkPSMCounts()[ 'looplink' ][ link.protein1 ][ link.protein1 ][ link.position2 ][ link.position1 ];
					} catch( err ) { }
				}

			} else if( link.type == 'monolink' ) {

				numPsms = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_linkPSMCounts()[ 'monolink' ][ link.protein1 ][ link.position1 ];	
...
ColorManager.validateLink = function( link ) {
	...
	if( !link.hasOwnProperty( "uid1" ) ) {
		...
		if( !imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_indexManager().containsUID( link.uid1 ) ) {
			throw Error( "invalid uid1 sent: " + link.uid1 );
		}
		
		if( !imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_imageProteinBarDataManager().getItemByUID( link.uid1 ) ) {
	...
	if( !link.hasOwnProperty( "position1" ) ) {
		...
		if( typeof link.position1 !== "number" ) {
			...
			var proteinIdForPosition1 = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_indexManager().getProteinIdForUID( uidForPosition1 );
			var proteinLengthForPosition1 = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_proteinLengths().getProteinLength( proteinIdForPosition1 );
	...
	if( link.type === "crosslink" || link.type ==="looplink" ) {
		...
		if( link.type === "crosslink" && !link.hasOwnProperty( "uid2" ) ) {
			...
			if( !imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_indexManager().containsUID( link.uid2 ) ) {
				throw Error( "invalid uid2 sent: " + link.uid2 );
			}
			if( !imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_imageProteinBarDataManager().getItemByUID( link.uid2 ) ) {
		...
		if( !link.hasOwnProperty( "position2" ) ) {
			...
			if( typeof link.position2 !== "number" ) {
				...
				var proteinIdForPosition2 = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_indexManager().getProteinIdForUID( uidForPosition2 );
				var proteinLengthForPosition2 = imagePagePrimaryRootCodeObject_LocalCopy.getVariable__v_proteinLengths().getProteinLength( proteinIdForPosition2 );
 ...
 */
var ColorManager_pass_imagePagePrimaryRootCodeObject = function( imagePagePrimaryRootCodeObject_LocalCopy_Param ) {

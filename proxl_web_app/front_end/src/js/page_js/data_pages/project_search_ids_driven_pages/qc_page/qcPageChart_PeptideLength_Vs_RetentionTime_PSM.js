/**
 * qcPageChart_PeptideLength_Vs_RetentionTime_PSM.js
 * 
 * Javascript for the viewQC.jsp page - Chart Peptide Length Vs Retention Time - in PSM section
 * 
 * page variable qcPageChart_PeptideLength_Vs_RetentionTime_PSM
 * 
...
//	}
	...
	this.clearChart = function() {
		...
		var $PeptideLengthVsRetentionTimeBlock = $("#PeptideLengthVsRetentionTimeBlock");
		$PeptideLengthVsRetentionTimeBlock.empty();
		...
		if ( ! _anySearchesHaveScanDataYes ) {
			...
			for ( var index = 0; index < selectedLinkTypes.length; index++ ) {
				...
				var $chart_outer_container_jq =
					this._addChartOuterTemplate( { $chart_group_container_table_jq : $PeptideLengthVsRetentionTimeBlock } );
...
//					linkType : selectedLinkType, 
		...
		selectedLinkTypes.forEach( function ( currentArrayValue, index, array ) {
			...
			//  Add empty chart with Loading message
			var $chart_outer_container_jq =
				this._addChartOuterTemplate( { linkType : selectedLinkType, $chart_group_container_table_jq : $PeptideLengthVsRetentionTimeBlock } );
	...
	/**
	 * Load the data for chart; Specific Link Type
	 */
	this.load_Data_ForLinkType = function( params ) {
		var objectThis = this;
		...
		_activeAjax[ selectedLinkType ] = 
			$.ajax({
//				cache : false,
				url : contextPathJSVar + "/services/qc/dataPage/peptideLengthVsRetentionTime", // 
				...
				success : function( ajaxResponseData ) {
					try {
						...
						objectThis.load_Data_ForLinkTypeResponse( responseParams );
					} catch( e ) {
						reportWebErrorToServer.reportErrorObjectToServer( { errorException : e } );
						throw e;
					}
	 ...
	 */
	this.load_Data_ForLinkTypeResponse = function( params ) {
		var objectThis = this;
		...
		this._add_Chart( { dataForChart: dataForChart, colorAndbarColor: colorAndbarColor, $chartContainer : $chart_container_jq } );
...
//		_chart_isLoaded = _IS_LOADED_YES;
	 ...
	 * 
	 */
	this._add_Chart = function( params ) {
		var dataForChart = params.dataForChart;
...
//		var numScans = dataForChart.numScans;
		...
		//  Not used
//		var peptideLengthMin = dataForChart.peptideLengthMin;
//		var peptideLengthMax = dataForChart.peptideLengthMax;
//		var retentionTimeBinMin = dataForChart.retentionTimeBinMin;
//		var retentionTimeBinMax = dataForChart.retentionTimeBinMax;
//		var retentionTimePossibleMax = dataForChart.retentionTimePossibleMax;
...
//								opacityString = ;
				...
				if ( maxPeptideLength === null || peptideLength > maxPeptideLength ) {
					maxPeptideLength = peptideLength;
				}
				if ( minPeptideLength === null || peptideLength < minPeptideLength ) {
					minPeptideLength = peptideLength;
				}
...
//		var vAxisTicks = 
//			this._get_Chart_Vertical_TickMarks( { 
//				maxValue : maxPeptideLength, minValue : minPeptideLength, dataPointCount : dataPointCount } );

		var hAxisTicks = 
			this._get_Chart_Horizontal_TickMarks( { maxValue : maxRetenionTimeError, minValue : minRetenionTimeError } );
...
//				,ticks: [-100,0,100]
				...
				vAxis: { title: 'Peptide Length', titleTextStyle: { color: 'black', fontSize: _PPM_Error_Vs_RetentionTime_For_PSMs_CHART_GLOBALS._AXIS_LABEL_FONT_SIZE }
//				,baseline: 0     // always start at zero
//				,ticks: vAxisTicks
				,maxValue : maxPeptideLength
				,minValue : minPeptideLength
				},
...
//		chartFullsize.draw( data, google.charts.Scatter.convertOptions( optionsFullsize ) );
	...
	/**
	 * !!!  re-write if use.  Not designed for peptide length
	 * 
	 * for vertical axis
	 */
//	this._get_Chart_Vertical_TickMarks = function( params ) {
...
//	};
	 ...
	 */
	this._get_Chart_Horizontal_TickMarks = function( params ) {
		var maxValue = params.maxValue;
		var minValue = params.minValue;
...
var qcPageChart_PeptideLength_Vs_RetentionTime_PSM = new QCPageChart_PeptideLength_Vs_RetentionTime_PSM();

export { qcPageChart_PeptideLength_Vs_RetentionTime_PSM }

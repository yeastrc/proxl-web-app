...
/**
 * 
 * Download data for QC Chart Summary Protein Data
 */
public class DownloadQC_Summary_ProteinChartDataAction {
	
	private static final Logger log = LoggerFactory.getLogger( DownloadQC_Summary_ProteinChartDataAction.class);
...
//			Collections.sort( searchIds );
			...
			try {
				...
				List<QC_SummaryCountsResultsPerLinkType_Merged> proteinCountPerLinkTypeList = qc_SummaryCountsResults_Merged.getProteinCountPerLinkTypeList();
				
				List<String> linkTypesList = new ArrayList<>( proteinCountPerLinkTypeList.size() );
				for ( QC_SummaryCountsResultsPerLinkType_Merged resultPerLinkType : proteinCountPerLinkTypeList ) {
				...
				// generate file name
				String filename = "proxl-qc-summary-protein-count-search-"
				...
				for ( QC_SummaryCountsResultsPerLinkType_Merged resultPerLinkType : proteinCountPerLinkTypeList ) {
					...
					if ( resultPerLinkType.isCombinedEntry() ) {
						linkType = "combined";  //  Hard coded in JS
					}
//					boolean dataFound;

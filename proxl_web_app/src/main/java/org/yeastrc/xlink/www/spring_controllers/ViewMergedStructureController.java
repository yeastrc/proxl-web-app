...
/**
 * Spring MVC controller for path /structure.do
 * (The 'NoSearchIds' forward was never returned by the action, so it is not reproduced.)
 */
@Controller
public class ViewMergedStructureController {

	private static final Logger log = LoggerFactory.getLogger( ViewMergedStructureController.class );

	@RequestMapping( A__SpringMVC_Controller_Paths.ViewMergedStructureController_structure )

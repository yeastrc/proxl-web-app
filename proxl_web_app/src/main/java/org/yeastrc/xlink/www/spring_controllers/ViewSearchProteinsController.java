...
/**
 * Spring MVC controller for paths /crosslinkProtein.do and /looplinkProtein.do
 *
 * <p>The {@code parameter} attribute ("crosslink" / "looplink"),
 * previously read via {@code mapping.getParameter()}, is now passed explicitly from each
 * mapped method into the shared handler.
 */
@Controller
public class ViewSearchProteinsController {

	private static final Logger log = LoggerFactory.getLogger( ViewSearchProteinsController.class );

	@RequestMapping( A__SpringMVC_Controller_Paths.ViewSearchProteinsController_crosslinkProtein )
	public String crosslinkProtein(
			@ModelAttribute( "searchViewProteinForm" ) SearchViewProteinsForm form,
			HttpServletRequest request,
			HttpServletResponse response ) throws Exception {
		return handle( SpringMvc_Config_Parameter_Values_Constants.PARAMETER__CROSSLINK, "viewSearchCrosslinkProtein", form, request, response );
	}

	@RequestMapping( A__SpringMVC_Controller_Paths.ViewSearchProteinsController_looplinkProtein )
	public String looplinkProtein(
			@ModelAttribute( "searchViewProteinForm" ) SearchViewProteinsForm form,
			HttpServletRequest request,
			HttpServletResponse response ) throws Exception {
		return handle( SpringMvc_Config_Parameter_Values_Constants.PARAMETER__LOOPLINK, "viewSearchLooplinkProtein", form, request, response );
	}

	private String handle(
			String actionMappingParameter,
			String successViewName,
			SearchViewProteinsForm form,
			HttpServletRequest request,
			HttpServletResponse response ) throws Exception {
		...
		try {
			...
			////  Put queryJSON for Peptide Link on the page
			{
				try {
					...
					} else if ( SpringMvc_Config_Parameter_Values_Constants.PARAMETER__LOOPLINK.equals( actionMappingParameter ) ) {

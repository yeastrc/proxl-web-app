...
/**
 * Spring MVC controller for path /peptide.do
 *
 * <p>The model attribute name "searchViewPeptideForm" matches the old Struts form-bean
 * name so it is exposed as the same request attribute the JSP expects.
 */
@Controller
public class ViewSearchPeptidesController {

	private static final Logger log = LoggerFactory.getLogger( ViewSearchPeptidesController.class );

	//  Spring equivalents of the Struts global-forwards (struts-config.xml).
	//  All are redirect="false" except account_disabled, matching the original config.
	private static final String FWD_NO_USER_SESSION = "forward:/user_loginPage.do";
	private static final String FWD_INSUFFICIENT_ACCESS_PRIVILEGE = "forward:/user_insufficient_access_privilege.do";
	private static final String FWD_INVALID_REQUEST_SEARCHES_ACROSS_PROJECTS = "forward:/invalidRequestSearchesAcrossProjects.do";
	private static final String FWD_INVALID_REQUEST_DATA = "forward:/invalidRequestData.do";

	@RequestMapping( A__SpringMVC_Controller_Paths.ViewSearchPeptidesController_peptide )
	public String peptide(
			@ModelAttribute( "searchViewPeptideForm" ) SearchViewPeptidesForm form,
		...
		try {
			...
			if ( projectIdsFromSearchIds.isEmpty() ) {
				...
				return FWD_INVALID_REQUEST_DATA;
			}
			if ( projectIdsFromSearchIds.size() > 1 ) {
				//  Invalid request, searches across projects
				return FWD_INVALID_REQUEST_SEARCHES_ACROSS_PROJECTS;
			}
			...
			if ( ! authAccessLevel.isPublicAccessCodeReadAllowed() ) {
				...
				if ( accessAndSetupWebSessionResult.isNoSession() ) {
					//  No User session
					return FWD_NO_USER_SESSION;
				}
				return FWD_INSUFFICIENT_ACCESS_PRIVILEGE;
			}

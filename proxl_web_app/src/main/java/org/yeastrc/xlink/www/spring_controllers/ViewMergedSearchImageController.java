...
/**
 * Spring MVC controller for path /image.do
 * (The 'NoSearchIds' forward was never returned by the action, so it is not reproduced.)
 */
@Controller
public class ViewMergedSearchImageController {

	private static final Logger log = LoggerFactory.getLogger( ViewMergedSearchImageController.class );

	@RequestMapping( A__SpringMVC_Controller_Paths.ViewMergedSearchImageController_image )
	public String image(

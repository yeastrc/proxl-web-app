...
@Path("/savedView")
public class SavedView_ChangeLabel_Service {
	
	private static final Logger log = LoggerFactory.getLogger( SavedView_ChangeLabel_Service.class);
		...
		try {
			...
			if ( authAccessLevel.isProjectOwnerAllowed() ) {
				...
				if ( dataPageSavedViewDTO.getAuthUserIdCreated() != authUserId ) {
    				log.warn( "Access Error: Researcher can only change label on own saved views, throwing NOT_AUTHORIZED_STATUS_CODE. savedViewId: " + savedViewId + ". userId: " + authUserId );
    				throw new WebApplicationException(
    						Response.status( WebServiceErrorMessageConstants.NOT_AUTHORIZED_STATUS_CODE )  //  Send HTTP code
    						.entity( WebServiceErrorMessageConstants.NOT_AUTHORIZED_TEXT ) // This string will be passed to the client
    						.build()
    		...
    		DataPageSavedViewDAO.getInstance().updateLabel( labelText, authUserId, savedViewId );

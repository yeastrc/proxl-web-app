...
/**
 * Spring MVC controller for paths /qc.do and /qc_Alex.do
 *
 * <p>The {@code parameter} attribute ("Alex" for /qc_Alex,
 * none for /qc), previously read via {@code mapping.getParameter()}, becomes the
 * {@code parameterAlex} flag passed from each mapped method.
 *
 * <p>SuccessSingle (1 search) -> viewQC.jsp ; SuccessMerged (&gt;1 search) -> viewQCMerged.jsp
 */
@Controller
public class ViewMergedSearchQCController {

	private static final Logger log = LoggerFactory.getLogger( ViewMergedSearchQCController.class );

	@RequestMapping( A__SpringMVC_Controller_Paths.ViewMergedSearchQCController_qc )
	public String qc(
			@ModelAttribute( "mergedSearchViewProteinForm" ) MergedSearchViewProteinsForm form,
			HttpServletRequest request,
			HttpServletResponse response ) throws Exception {
		return handle( false, form, request, response );
	}

	@RequestMapping( A__SpringMVC_Controller_Paths.ViewMergedSearchQCController_qcAlex )
	public String qcAlex(
			@ModelAttribute( "mergedSearchViewProteinForm" ) MergedSearchViewProteinsForm form,
			HttpServletRequest request,
			HttpServletResponse response ) throws Exception {
		return handle( true, form, request, response );
	}

	private String handle(
			boolean parameterAlex,
		...
		try {
			request.setAttribute( "parameterAlex", parameterAlex );

...
/**
 * 
 * Download data for QC Chart Scan File MS1 VS M/Z Data
 */
public class DownloadQC_MS1_VS_M_Over_Z_ChartDataAction {
	...
	public String execute( SingleRequestJSONStringFieldForm form, HttpServletRequest request, HttpServletResponse response )
					  throws Exception {
		...
		try {
			...
			if ( projectIdsFromSearchIds.isEmpty() ) {
				...
				log.error( msg );
				return SpringMvcGlobalForwardNames.INVALID_REQUEST_DATA;
			}
			if ( projectIdsFromSearchIds.size() > 1 ) {
				//  Invalid request, searches across projects
				return SpringMvcGlobalForwardNames.INVALID_REQUEST_SEARCHES_ACROSS_PROJECTS;
			}
			...
			List<Map.Entry<Long, MutableDouble>> ms1_IntensitiesBinnedSummedMappedBy_M_Over_Z_List = new ArrayList<>( ms1_IntensitiesBinnedSummedMappedBy_M_Over_Z.size() );
			for ( Map.Entry<Long, MutableDouble> entry : ms1_IntensitiesBinnedSummedMappedBy_M_Over_Z.entrySet() ) {
				ms1_IntensitiesBinnedSummedMappedBy_M_Over_Z_List.add( entry );
			}
			Collections.sort( ms1_IntensitiesBinnedSummedMappedBy_M_Over_Z_List, new Comparator<Map.Entry<Long, MutableDouble>>() {
				@Override
			...
			try {
				...
				// generate file name
				String filename = "proxl-qc-summary-scan-ms1-vs-m-over-z-search-"
						+ searchId
						+ "-scan-"
						+ scanFileId
						+ "-" + fmt.print( dt )
				...
				//  Write header line
				writer.write( "ION CURRENT SUMMED FOR BIN\tm/z BIN START (>=)\tm/z BIN END (<)\tSEARCH ID" );
				writer.write( "\n" );
				
				for ( Map.Entry<Long, MutableDouble> entry : ms1_IntensitiesBinnedSummedMappedBy_M_Over_Z_List ) {

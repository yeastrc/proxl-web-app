...
/**
 * 
 * Download data for QC Chart Digestion Fraction Peptides w/ Missed Cleavages Data
 */
public class DownloadQC_Digestion_PeptideMissedCleavageChartDataAction {
	
	private static final Logger log = LoggerFactory.getLogger( DownloadQC_Digestion_PeptideMissedCleavageChartDataAction.class);
...
//			Collections.sort( searchIds );
			...
			try {
				...
				List<QC_MissingCleavageReportedPeptidesCountResultsPerLinkType_Merged> peptideCountPerLinkTypeList =
						qc_MissingCleavageReportedPeptidesCount_Merged_Results.getPeptideCountPerLinkTypeList();
				
				List<String> linkTypesList = new ArrayList<>( peptideCountPerLinkTypeList.size() );
				for ( QC_MissingCleavageReportedPeptidesCountResultsPerLinkType_Merged resultPerLinkType : peptideCountPerLinkTypeList ) {
				...
				// generate file name
				String filename = "proxl-qc-digestion-peptides-missed-cleavages-search-"
				...
				//  Write header line
				writer.write( "FRACTION\tCOUNT\tTOTAL COUNT\tLINK TYPE\tSEARCH ID" );
				writer.write( "\n" );

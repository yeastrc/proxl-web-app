...
import org.yeastrc.xlink.www.spring_controllers__helpers.ProteinsMergedCommonPageDownload;
...
import org.yeastrc.xlink.www.spring_controllers__helpers.ProteinsMergedCommonPageDownload.ProteinsMergedCommonPageDownloadResult;
 ...
 *
 */
public class DownloadMergedProteinsXvisAction {
	
	private static final Logger log = LoggerFactory.getLogger( DownloadMergedSearchProteinsAction.class);
	
	public String execute( DownloadProteinCLMSForm form, HttpServletRequest request, HttpServletResponse response )
					  throws Exception {
		try {
			// our form
			...
			try {
				...
				// generate file name
				String filename = "proxl-xvis-export-";
				...
				if( !form.isCrosslinksOnly() ) {
					for( MergedSearchProteinLooplink link : looplinks ) {
						...
						line += name + ",";							// Protein2 (excluded for loop-links)
						//line += ",";								// PepPos2
						//line += ",";								// PepSeq2

...
public class SearchPsmSearcher {
	...
	private static final Logger log = LoggerFactory.getLogger( SearchPsmSearcher.class);
	...
	public Integer getSinglePsmId( int searchId, int reportedPeptideId  ) throws Exception {
		...
		ResultSet rs = null;
		final String sql = "SELECT id FROM psm WHERE search_id = ? AND reported_peptide_id = ? LIMIT 1 ";
		try {

...
// (function($) {
	...
	$.fn.specview = function (opts) {
        ...
        var defaults = {
                ...
                massError: 0.01, // mass tolerance (in th) for labeling peaks
                extraPeakSeries:[],
                showIonTable: true,
                ...
                loopLinkDataInputFormat: null,  // Data for a Loop Link, Input to Lorikeet Format
                //  Internal object stored as options.loopLinkData
                
                crossLinkDataInputFormat: null   // Data for a Cross Link, Input to Lorikeet Format
                //  Internal object stored as options.crossLinkData
        };
    ...
    var elementIds = {
            ...
            fileinfo: "fileinfo",
            seqinfo: "seqinfo",
            seqMassInfo: "seqMassInfo",
            linkerMassInfo: "linkerMassInfo",
    ...
    function init(parent_container, options) {
        ...
        if ( options.loopLinkDataInputFormat ) {

        	options.loopLinkData  = new LoopLinkData( options.loopLinkDataInputFormat );
        }
        ...
        if ( options.crossLinkDataInputFormat ) {
        	...
        	options.crossLinkData  = new CrossLinkData( options.crossLinkDataInputFormat, options );
        ...
        //  Run this code after timeout to redraw plot properly
			...
			if(options.sequence) {
				...
				for(var i = 1; i < sequence.length; i += 1) {
					...
					for(var j = 0; j < todoIonSeries.length; j += 1) {
						...
						if(tion.term == "n")
							ionSeriesData.push(sion = Ion.getSeriesIon(tion, options.peptide, i, options.loopLinkData, massType));
						else if(tion.term == "c")
							ionSeriesData.unshift(sion = Ion.getSeriesIon(tion, options.peptide, i, options.loopLinkData, massType));
					}
				}
			}
		}
    ...
    function getPeaks(container)
    {
        ...
        var peaksIntermediate = undefined;
        ...
        if($(getElementSelector(container, elementIds.peakDetect)).is(":checked"))
        {
            ...
            peaksIntermediate = options.sparsePeaks;
        }
        else
        {
        	peaksIntermediate = options.peaks;
        }


        //   Remove all zero intensity peaks

        var peaksToReturn = [];

        for(var i = 0; i < peaksIntermediate.length; i++ )
        {
            if(peaksIntermediate[i][1] != 0.0)
            {
            	peaksToReturn.push( peaksIntermediate[i] );
            }
        }

        return peaksToReturn;
	...
	function initContainer(container) {
        ...
        if(options.showIonTable) {
            ...
            // placeholder for sequence mass
			parentTable += '<div id="'+getElementId(container, elementIds.seqMassInfo)+'" style="width:100%;"></div> ';

            // placeholder for linker mass
			parentTable += '<div id="'+getElementId(container, elementIds.linkerMassInfo)+'" style="width:100%;"></div> ';
	...
	function makeIonTable(container) {
        ...
        var loopLinkPos1ZeroBased = null;
        var loopLinkPos2ZeroBased = null;
        
        if ( options.loopLinkData ) {
        
        	loopLinkPos1ZeroBased = options.loopLinkData.loopLinkPos1 - 1;
        	loopLinkPos2ZeroBased = options.loopLinkData.loopLinkPos2 - 1;
		...
		for(var i = 0; i < options.sequence.length; i += 1) {
			...
			if ( options.loopLinkData ) {
				...
				if ( i > loopLinkPos1ZeroBased && i <= loopLinkPos2ZeroBased ) {

					var ionEntry = ionArray[ ionArrayIndex ];
					
					ionEntry.ionChars += aaChar;  // append to existing string
					ionEntry.lastPosition = i;
				
				} else {
					ionArrayIndex++;

					ionArray[ ionArrayIndex ] = { ionChars: aaChar, firstPosition: i, lastPosition: i };
		...
		var aminoAcidPosition = 0;  // The position of each individual amino acid, used for variable mods, One based
		
		for ( var i = 0; i < ionArray.length; i++ ) {
			
//			if ( i === ionArray.length -1 ) {
//				
//				var z = 0;
//			}

			var ionEntry = ionArray[ i ];

			var aaChar = ionEntry.ionChars;
...
//				if( ionPositionForward /* i */ < options.sequence.length - 1) {
				if( i < ionArray.length - 1) {
					var seriesData = getCalculatedSeries(ionSeries, ntermIons[n]);
					var cls = "";
					var style = "";
					if(seriesData[ ionEntry.lastPosition /* i */].match) {
						cls="matchIon";
						style="style='background-color:"+Ion.getSeriesColor(ntermIons[n])+";'";
					}
					myTable +=    "<td class='"+cls+"' "+style+" >" +round(seriesData[ ionEntry.lastPosition /* i */].mz)+  "</td>";  
				}
				else {
					myTable +=    "<td>" +"&nbsp;"+  "</td>"; 
				} 
			...
			//  Output the sequence for that ion number
			
			if ( aaChar.length === 1 ) {
				
				// Special case for single character, following the original processing for outputting the sequence
				
				aminoAcidPosition++;
				
				var varModsAtPositionCssClass = "";
				var crossLinkAtPositionCssClass = "";
				var crossLinkAtPositionTitle = "";
				
				if(options.peptide.varMods()[ aminoAcidPosition ])
					varModsAtPositionCssClass = "modified";
					
				if ( options.crossLinkData ) {
					
					var linkerPosition = options.crossLinkData.getLinkerPosition();
					
					if ( linkerPosition == aminoAcidPosition ) {
						
						crossLinkAtPositionCssClass = "cross-link-amino-acid";
						crossLinkAtPositionTitle = ' title="Cross Link at this position"';
					}
				}
				
					
				myTable += "<td class='seq " + varModsAtPositionCssClass  + " " + crossLinkAtPositionCssClass + "' "
						+ crossLinkAtPositionTitle + ">"+aaChar+"</td>";

				
			} else {

				myTable += "<td class='seq '>";

				for ( var aaCharIndex = 0; aaCharIndex < aaChar.length; aaCharIndex++ ) {

					aminoAcidPosition++;

					var aaCharAtAminoAcidPosition = aaChar.charAt( aaCharIndex );

					if ( options.peptide.varMods()[ aminoAcidPosition ] ) {

						myTable += "<span class='modified'>" + aaCharAtAminoAcidPosition + "</span>";
					} else {

						myTable += aaCharAtAminoAcidPosition;
					}
...
//				myTable += "<td class='seq'>"+aaChar+"</td>";
			...
			myTable += "<td class='numCell'>" + ( ionArray.length - i /* options.sequence.length - i */ ) + "</td>";
			
			// cterm ions
			for(var c = 0; c < ctermIons.length; c += 1) {
				if(i > 0) {
					var seriesData = getCalculatedSeries(ionSeries, ctermIons[c]);
					var idx = options.sequence.length - ionEntry.firstPosition - 1 /* options.sequence.length - i  - 1 */ ;
					var cls = "";
					var style = "";
...
//			ionPositionForward += aaChar.length();  // Tracking the position of this ion in the sequence forward
//			ionPositionBackward -=  aaChar.length();  // Tracking the position of this ion in the sequence backward
	...
	function showSequenceInfo (container) {
		...
		if(options.sequence) {
			...
			specinfo += ', <span style="white-space:nowrap;">MH+ '+mass.toFixed(4) + '</span>';
			if(mz) 
				specinfo += ', <span style="white-space:nowrap;">m/z '+mz.toFixed(4) + '</span>';
			specinfo += '</div>';
	...
	function makeViewingOptions(container) {
        ...
        var massErrorPlot_optionChecked = "";

        if(options.showMassErrorPlot === true)
        {
        	massErrorPlot_optionChecked = 'checked="checked"';
        }
		...
		// tooltip option
		+ '<nobr> '
		+ '<label><input id="'+getElementId(container, elementIds.enableTooltip)+'" type="checkbox">Enable tooltip</label> '
//		+ '</nobr> '

        // mass error plot option
//        + '<nobr>'
        + '<label><input id="'+getElementId(container, elementIds.massErrorPlot_option)+'" type="checkbox" '
      
        + massErrorPlot_optionChecked
        + '>Plot mass error</label> '
        + '</nobr>'
	...
	// Sequence Mass Info
	//---------------------------------------------------------
	function calculateSequenceMassAndPutOnPage(container) {
		...
		var $seqMassInfo = $(getElementSelector(container, elementIds.seqMassInfo));
		

		$seqMassInfo.empty();
		

		var crossLinkedPeptideAndLinkerMass = options.crossLinkData.getCrossLinkedPeptideAndLinkerMass( massType );
		
		var crossLinkedPeptideAndLinkerMassRounded = crossLinkedPeptideAndLinkerMass.toFixed(2); 

		var text = "Linked Peptide + Linker Mass: " + crossLinkedPeptideAndLinkerMassRounded;


		$seqMassInfo.text( text );
